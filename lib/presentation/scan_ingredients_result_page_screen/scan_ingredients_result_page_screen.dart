import 'models/scan_ingredients_result_page_model.dart';
...
class ScanIngredientsResultPageScreen extends StatefulWidget {
  const ScanIngredientsResultPageScreen({Key? key}) : super(key: key);
  ...
  static Widget builder(BuildContext context) {
    return ChangeNotifierProvider(
        ...
        child: ScanIngredientsResultPageScreen());
  }
}
...
class ScanIngredientsResultPageScreenState
  ...
  Widget _buildResultOutput(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text("msg_scanner_s_result".tr, style: CustomTextStyles.titleSmallMedium),
      SizedBox(height: 9.v),
      Selector<ScanIngredientsResultPageProvider, TextEditingController?>(
          selector: (context, provider) => provider.containerController,
          builder: (context, containerController, child) {
            return CustomTextFormField(controller: containerController);
          })
    ]);
  }
  ...
  Widget _buildWarningOutput(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text("lbl_warning".tr, style: CustomTextStyles.titleSmallMedium),
      SizedBox(height: 8.v),
      Selector<ScanIngredientsResultPageProvider, TextEditingController?>(
          selector: (context, provider) => provider.containerController1,
